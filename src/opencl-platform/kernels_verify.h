...
inline void ge_p1p1_to_p2(ge_p2 *r, const ge_p1p1 *p) {
    fe_mul(r->X, p->X, p->T);
    fe_mul(r->Y, p->Y, p->Z);
    fe_mul(r->Z, p->Z, p->T);
}
...
inline void ge_p3_dbl(ge_p1p1 *r, const ge_p3 *p) {
    ge_p2 q;
    ge_p3_to_p2(&q, p);
    ge_p2_dbl(r, &q);
}
...
static void
ed25519_sign_device(__global unsigned char *signature,
                   __global const unsigned char *message,
                   uint32_t message_len,
                   __global const unsigned char *public_key,
                   __global const unsigned char *private_key) {
    sha512_context hash;
    ...
    ge_p3_tobytes(signature_priv, &R);
	
	for(int i=0; i<64; i++) {
		signature[i] = signature_priv[i];
    }
	...
	unsigned char private_key_priv[32];
    for(int i=0; i<32; i++) {
        private_key_priv[i] = private_key[i];
    }
	
    sc_muladd(signature_priv + 32, hram, private_key_priv, r);
...
void ed25519_sign(__global unsigned char *signature,
                   __global const unsigned char *message,
                   uint32_t message_len,
                   __global const unsigned char *public_key,
                   __global const unsigned char *private_key) {
